...
type Simulator struct {
	...
	serversMarked map[int]int //Key = snapShotId Value = number of servers marked
	dummyChannel  chan int
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	// TODO: IMPLEMENT ME
	// This will call the specified snapshot at the server
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// TODO: IMPLEMENT ME
	//append to channel and block collect snap until all the servers have completed the snapshot
	sim.serversMarked[snapshotId] = sim.serversMarked[snapshotId] + 1
	numServers := len(sim.servers)
	if numServers == sim.serversMarked[snapshotId] {
		sim.dummyChannel <- 1
	}
}
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	...
	for _, server := range sim.servers {
		...
		// fmt.Println()
		// fmt.Println()
		serverStateMap := server.serverState
		// fmt.Printf("ServerStateMap is %s %v", server.Id, serverStateMap)
		// fmt.Println()
		serverState, _ := serverStateMap.Load(snapshotId)
		// fmt.Printf("ServerState is %v", serverState)
		// fmt.Println()
		serverStateCast := serverState.(SnapshotState)
		// fmt.Printf("ServerStateCast is %v", serverStateCast)
		// fmt.Println()
		token := serverStateCast.tokens[server.Id]
		// fmt.Printf("token is %d", token)
		// fmt.Println()
		// fmt.Println()
		snap.tokens[server.Id] = token
		message, ok := server.messageinChannel.Load(snapshotId)
		// fmt.Printf("Type of message is %t", message)
		// fmt.Println()
		if ok {
			// fmt.Printf("Message is %v", message)
			messageCast := message.([]*SnapshotMessage)
			//fmt.Printf("Message cast is %v", messageCast)
			//snap.messages = append(snap.messages, &messageCast)
			for _, message2 := range messageCast {
				snap.messages = append(snap.messages, message2)
			}
		}
	}
	fmt.Printf("Snap is %v", snap)
	return &snap
